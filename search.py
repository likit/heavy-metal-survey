...
db = pymongo.Connection()['tmp']

api_key = os.environ['SCOPUS_APIKEY']

'''User defined variables.'''
item_per_page = 200
pub_year = int(sys.argv[1])
outfile_prefix = 'heavy metal'
query = 'tak(heavy metal) AND tak(sensor) AND pub-date IS %d' % pub_year
sleeptime = 5
...
for start in range(0,total_results+1, item_per_page):
    ...
    for n, entry in enumerate(articles['entry']):
        print >> sys.stderr, '\tdownloading %s...' % entry['dc:title'][:80]
